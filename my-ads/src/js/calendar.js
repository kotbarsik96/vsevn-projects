...
const availableDates = [
    [new Date('2/20/2023'), new Date('3/15/2023')],
...
function CalendarPopup(container, adjustPosition = true) {
    ...
    container.setAttribute('data-placement', attr);
...
function DoubleCalendar(opts) {
    CalendarPopup.call(this, opts.container, opts.adjustPosition);
...
function Calendar(opts) {
    ...
    const yearSelect = new Select({
        container: $calendar.querySelector('.calendar__year .select-container'),
        values: Array.from(Array(20).keys()).map(i => MAX_YEAR - i)
    });
    const monthSelect = new Select({
        container: $calendar.querySelector('.calendar__month .select-container'),
        values: monthDefault
    });

    const yearPrevBtn = $calendar.querySelector('.calendar__year-prev');
    const yearNextBtn = $calendar.querySelector('.calendar__year-next');
    const monthPrevBtn = $calendar.querySelector('.calendar__month-prev');
    const monthNextBtn = $calendar.querySelector('.calendar__month-next');
    ...
    function setYear(year) {
        ...
        currentYear = year;
        yearSelect.selectByIndex(MAX_YEAR - year, false);
    }
    ...
    function setMonth(month) {
        ...
        currentMonth = month;
        monthSelect.selectByIndex(month, false);
    }
    ...
    yearSelect.setOnSelect((value, index) => {
        setYear(+value);
        updateBtnsState();
        renderDays();
    });
    monthSelect.setOnSelect((value, index) => {
        setMonth(+index);
        updateBtnsState();
        renderDays();
    });
